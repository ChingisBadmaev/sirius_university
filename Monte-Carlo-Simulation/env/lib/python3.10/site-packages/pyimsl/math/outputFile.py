...
###########################################################################
from pyimsl.util.imslUtils import MATH, checkForList, fatalErrorCheck, loadimsl
...
def outputFile(setOutputFile=None, getOutputFile=None, setErrorFile=None, getErrorFile=None):
    imslmath.imsl_output_file.restype = None
    shape = []
    evalstring = 'imslmath.imsl_output_file('
    if not (setOutputFile is None):
        evalstring += repr(IMSL_SET_OUTPUT_FILE)
        evalstring += ','
        evalstring += 'c_void_p(setOutputFile)'
    if not (getOutputFile is None):
        if(evalstring.count(',') > 0):
            evalstring += ','
        evalstring += repr(IMSL_GET_OUTPUT_FILE)
    ...
    if not (setErrorFile is None):
        evalstring += ','
        evalstring += repr(IMSL_SET_ERROR_FILE)
        evalstring += ','
        evalstring += 'struct(setErrorFile)'
    if not (getErrorFile is None):
        if(evalstring.count(',') > 0):
            evalstring += ','
        evalstring += repr(IMSL_GET_ERROR_FILE)
        checkForList(getErrorFile, 'getErrorFile')
        evalstring += ','
        getErrorFile_pefile_tmp = c_voip_p()
