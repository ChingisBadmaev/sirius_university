...
def receivedMaturity(settlement, maturity, investment, discountRate, basis):
    """ Evaluates the amount one receives when a fully invested security reaches the maturity date.
    """
    imslmath.imsl_d_received_maturity.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_d_received_maturity('
    ...
    evalstring += ','
    evalstring += 'c_double(discountRate)'
    evalstring += ','
