...
def modifiedInternalRate(values, financeRate, reinvestRate):
    """ Evaluates the modified internal rate of return for a schedule of periodic cash flows.
    """
    imslmath.imsl_d_modified_internal_rate.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_d_modified_internal_rate('
    ...
    count = shape[0]
    evalstring += ','
    evalstring += 'c_double(financeRate)'
    evalstring += ','
    evalstring += 'c_double(reinvestRate)'
    evalstring += ')'
