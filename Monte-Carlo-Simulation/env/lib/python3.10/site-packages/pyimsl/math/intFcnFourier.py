...
def intFcnFourier(fcn, a, weight, omega, errAbs=None, errEst=None, maxSubinter=None, maxCycles=None, maxMoments=None, nCycles=None, nEvals=None, fcnWData=None):
    """ Computes a Fourier sine or cosine transform.
    """
    imslmath.imsl_d_int_fcn_fourier.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_d_int_fcn_fourier('
    ...
    if not (maxCycles is None):
        evalstring += ','
        evalstring += repr(IMSL_MAX_CYCLES)
        evalstring += ','
        evalstring += 'c_int(maxCycles)'
    if not (maxMoments is None):
        evalstring += ','
        evalstring += repr(IMSL_MAX_MOMENTS)
        evalstring += ','
        evalstring += 'c_int(maxMoments)'
    if not (nCycles is None):
        evalstring += ','
        evalstring += repr(IMSL_N_CYCLES)
        checkForList(nCycles, 'nCycles')
        evalstring += ','
        nCycles_nCycles_tmp = c_int()
        evalstring += 'byref(nCycles_nCycles_tmp)'
    ...
    if not (nCycles is None):
        processRet(nCycles_nCycles_tmp, shape=1, pyvar=nCycles)
    if not (nEvals is None):
