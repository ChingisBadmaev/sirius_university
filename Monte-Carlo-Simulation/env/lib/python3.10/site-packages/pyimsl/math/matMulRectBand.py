...
###########################################################################
from pyimsl.util.imslUtils import MATH, checkForList, checkForDict, checkForStr, fatalErrorCheck, loadimsl, processRet, toNumpyArray, toByte
from numpy import double, dtype, matrix, shape, size
...
def matMulRectBand(string, aMatrix=None, bMatrix=None, xVector=None, returnMatrixCodiagonals=None):
    """ Computes the transpose of a matrix, a matrix-vector product, or a matrix-matrix product, all matrices stored in band form.
    """
    imslmath.imsl_d_mat_mul_rect_band.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslmath.imsl_d_mat_mul_rect_band('
    ...
    if not (aMatrix is None):
        ...
        aMatrix_a_tmp = toNumpyArray(aMatrix_a_tmp, 'a', shape=shape, dtype='double', expectedShape=(
    ...
    if not (bMatrix is None):
        ...
        bMatrix_b_tmp = toNumpyArray(bMatrix_b_tmp, 'b', shape=shape, dtype='double', expectedShape=(
