...
###########################################################################
from pyimsl.util.imslUtils import MATH, checkForBoolean, checkForList, d_complex, fatalErrorCheck, loadimsl, toNumpyArray, processRet
...
def vectorNormComplex(x, oneNorm=None, infNorm=None, secondVector=None):
    """ Computes various norms of a vector or the difference of two vectors.
    """
    imslmath.imsl_z_vector_norm.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_z_vector_norm('
    evalstring += 'c_int(n)'
    evalstring += ','
    ...
    if not (secondVector is None):
        ...
        secondVector = toNumpyArray(
            secondVector, 'secondVector', shape=shape, dtype='d_complex', expectedShape=(n))
        evalstring += 'secondVector'
