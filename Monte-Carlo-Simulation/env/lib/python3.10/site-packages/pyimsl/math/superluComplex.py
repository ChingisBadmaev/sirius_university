...
from .mathStructs import Imsl_c_super_lu_factor
...
def superluComplex(a, b, equilibrate=None, columnOrderingMethod=None, colpermVector=None, transpose=None, iterativeRefinement=None, factorSolve=None, diagPivotThresh=None, symmetricMode=None, performanceTuning=None, cscFormat=None, supplySparseLuFactor=None, returnSparseLuFactor=None, condition=None, pivotGrowthFactor=None, forwardErrorBound=None, backwardError=None):
    ...
    """
    imslmath.imsl_z_superlu.restype = POINTER(d_complex)
    shape = []
    evalstring = 'imslmath.imsl_z_superlu('
    evalstring += 'c_int(n)'
    evalstring += ','
    evalstring += 'c_int(nz)'
    evalstring += ','
    ...
    if not (cscFormat is None):
        ...
        cscFormat_hbValues_tmp = toNumpyArray(
            cscFormat_hbValues_tmp, 'hbValues', shape=shape, dtype='struct', expectedShape=(nz))
    ...
    if not (returnSparseLuFactor is None):
        ...
        evalstring += ','
        # returnSparseLuFactor_luFactorReturned_tmp = struct()
        # evalstring += 'byref(returnSparseLuFactor_luFactorReturned_tmp)'
        returnSparseLuFactor_luFactorReturned_tmp = (Imsl_c_super_lu_factor)()
        evalstring += 'byref(returnSparseLuFactor_luFactorReturned_tmp)'
