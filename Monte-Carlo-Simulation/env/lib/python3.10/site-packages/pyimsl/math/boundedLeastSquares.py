...
from numpy import double, dtype, shape, size, empty, zeros
...
def boundedLeastSquares(fcn, m, n, ibtype, xlb, xub, xguess=None, jacobian=None,
                        xscale=None, fscale=None, gradTol=None, stepTol=None, relFcnTol=None, absFcnTol=None,
                        maxStep=None, initTrustRegion=None, goodDigit=None, maxItn=None, maxFcn=None,
                        maxJacobian=None, internScale=None, fvec=None, fjac=None, fjacColDim=None):
    ...
    """
    imslmath.imsl_d_bounded_least_squares.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslmath.imsl_d_bounded_least_squares('
    ...
    evalstring += ','
    if (ibtype == 1) or (ibtype == 2):  # used for output only
        checkForList(xlb, 'xlb')
        xlb_xlb_tmp = empty(n, dtype=double)
        checkForList(xub, 'xub')
        xub_xub_tmp = empty(n, dtype=double)
    elif ibtype == 0:  # input param
        xlb_xlb_tmp = toNumpyArray(
            xlb, 'xlb', shape=shape, dtype='double', expectedShape=(n))
        xub_xub_tmp = toNumpyArray(
            xub, 'xub', shape=shape, dtype='double', expectedShape=(n))
    elif ibtype == 3:  # inout param - only first index is used/needed but we create the entire array
        xlb_xlb_tmp = zeros(n, dtype=double)
        xlb_xlb_tmp[0] = xlb[0]
        xub_xub_tmp = zeros(n, dtype=double)
        xub_xub_tmp[0] = xub[0]
    evalstring += 'xlb_xlb_tmp.ctypes.data_as(c_void_p)'
    evalstring += ','
    evalstring += 'xub_xub_tmp.ctypes.data_as(c_void_p)'
    ...
    if (ibtype != 0):  # output or inout params
        processRet(xlb_xlb_tmp, shape=(n), pyvar=xlb)
        processRet(xub_xub_tmp, shape=(n), pyvar=xub)
    if not (fvec is None):
