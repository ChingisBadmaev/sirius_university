...
def modifiedMethodOfLines(t, tend, xbreak, y, state, fcnUt, fcnBc):
    ...
    if (not(isinstance(state[0], CnlState))):
        ...
        raise ValueError(errStr)
    imslmath.imsl_d_modified_method_of_lines.restype = None
    shape = []
    evalstring = 'imslmath.imsl_d_modified_method_of_lines('
