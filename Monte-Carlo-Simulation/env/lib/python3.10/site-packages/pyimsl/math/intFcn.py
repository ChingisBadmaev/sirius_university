...
def intFcn(fcn, a, b, rule=None, errAbs=None, errRel=None, errEst=None, maxSubinter=None, nSubinter=None, nEvals=None, fcnWData=None):
    """ Integrates a function using a globally adaptive scheme based on Gauss-Kronrod rules.
    """
    imslmath.imsl_d_int_fcn.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_d_int_fcn('
    ...
    if not (rule is None):
        evalstring += ','
        evalstring += repr(IMSL_RULE)
        evalstring += ','
        evalstring += 'c_int(rule)'
