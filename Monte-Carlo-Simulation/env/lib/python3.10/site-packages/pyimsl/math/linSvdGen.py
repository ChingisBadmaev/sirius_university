...
###########################################################################
from pyimsl.util.imslUtils import MATH, checkForList, fatalErrorCheck, loadimsl, processRet, toNumpyArray, checkForDict
from numpy import double, dtype, min, rank, shape, size
...
def linSvdGen(a, rank=None, u=None, v=None, inverse=None):
    ...
    """
    imslmath.imsl_d_lin_svd_gen.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslmath.imsl_d_lin_svd_gen('
    evalstring += 'c_int(m)'
    evalstring += ','
    evalstring += 'c_int(n)'
    evalstring += ','
    ...
    if not (u is None):
        ...
        checkForList(u, 'u')
        evalstring += ','
        u_pU_tmp = POINTER(c_double)(c_double())
    ...
    if not (v is None):
        ...
        checkForList(v, 'v')
        evalstring += ','
        v_pV_tmp = POINTER(c_double)(c_double())
    ...
    if not (inverse is None):
        ...
        evalstring += ','
        inverse_pGenInva_tmp = POINTER(c_double)(c_double())
