...
def intFcnSingPts(fcn, a, b, points, errAbs=None, errRel=None, errEst=None, maxSubinter=None, nSubinter=None, nEvals=None, fcnWData=None):
    """ Integrates a function with singularity points given.
    """
    imslmath.imsl_d_int_fcn_sing_pts.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_d_int_fcn_sing_pts('
    ...
    evalstring += ','
    evalstring += 'c_int(npoints)'
    evalstring += ','
    points = toNumpyArray(points, 'points', shape=shape,
                          dtype='double', expectedShape=(0))
    evalstring += 'points.ctypes.data_as(c_void_p)'
    npoints = shape[0]
