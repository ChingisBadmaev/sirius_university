...
def linSolGenBandComplex(a, nlca, nuca, b, transpose=None, factor=None, condition=None, factorOnly=None, solveOnly=None):
    imslmath.imsl_z_lin_sol_gen_band.restype = POINTER(d_complex)
    shape = []
    evalstring = 'imslmath.imsl_z_lin_sol_gen_band('
    evalstring += 'c_int(n)'
    evalstring += ','
    if solveOnly:
        evalstring += 'c_void_p()'
    else:
        a = toNumpyArray(a, 'a', shape=shape,
                         dtype='d_complex', expectedShape=(0, 0))
        evalstring += 'a'
        n = shape[1]
    evalstring += ','
