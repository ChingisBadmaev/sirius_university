...
def yearFraction(start_date, end_date, basis):
    """ Evaluates the fraction of a year represented by the number of whole days between two dates.
    """
    imslmath.imsl_d_year_fraction.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_d_year_fraction('
    evalstring += 'start_date'
    evalstring += ','
    evalstring += 'end_date'
    evalstring += ','
    evalstring += 'c_int(basis)'
    evalstring += ')'
    start_date = dateConvert(start_date, toStructTm=True)
    end_date = dateConvert(end_date, toStructTm=True)
