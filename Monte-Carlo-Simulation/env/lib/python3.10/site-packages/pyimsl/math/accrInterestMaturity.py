...
def accrInterestMaturity(issue, maturity, couponRate, parValue, basis):
    """ Evaluates the interest which has accrued on a security that pays interest at maturity.
    """
    imslmath.imsl_d_accr_interest_maturity.restype = c_double
    shape = []
    evalstring = 'imslmath.imsl_d_accr_interest_maturity('
    evalstring += 'issue'
    evalstring += ','
    evalstring += 'maturity'
    evalstring += ','
