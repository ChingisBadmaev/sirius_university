...
from numpy import complex, dtype, int, shape, size, transpose
from ctypes import POINTER, byref, c_double, c_int, c_void_p
from .mathStructs import d_complex
from .mathStructs import d_complex
from .mathStructs import d_complex
from pyimsl.math.mathStructs import Imsl_c_sparse_lu_factor
from pyimsl.math.mathStructs import Imsl_c_sparse_lu_factor
...
def linSolGenCoordinateComplex(a, b, returnSparseLuFactor=None, supplySparseLuFactor=None, freeSparseLuFactor=None, returnSparseLuInCoord=None, supplySparseLuInCoord=None, factorOnly=None, solveOnly=None, transpose=None, condition=None, pivotingStrategy=None, numberOfSearchRows=None, iterativeRefinement=None, dropTolerance=None, hybridFactorization=None, growthFactorLimit=None, growthFactor=None, smallestPivot=None, numNonzerosInFactor=None, cscFormat=None, memoryBlockSize=None):
    ...
    """
    imslmath.imsl_z_lin_sol_gen_coordinate.restype = POINTER(d_complex)
    shape = []
    evalstring = 'imslmath.imsl_z_lin_sol_gen_coordinate('
    evalstring += 'c_int(n)'
    evalstring += ','
    evalstring += 'c_int(nz)'
    evalstring += ','
    ...
    if not (returnSparseLuFactor is None):
        ...
        evalstring += ','
        returnSparseLuFactor_luFactor_tmp = (Imsl_c_sparse_lu_factor)()
    ...
    if not (returnSparseLuInCoord is None):
        ...
        returnSparseLuInCoord_luCoordinate_tmp = POINTER(
            Imsl_c_sparse_elem)(Imsl_c_sparse_elem())
    ...
    if not (supplySparseLuInCoord is None):
        ...
        supplySparseLuInCoord_luCoordinate_tmp = toNumpyArray(
            supplySparseLuInCoord_luCoordinate_tmp, 'luCoordinate', shape=shape, dtype='Imsl_c_sparse_elem', expectedShape=(0, 3))
    ...
    if not (cscFormat is None):
        ...
        checkForList(cscFormat, 'cscFormat', size=3)
        evalstring += ','
        cscFormat_colPtr_tmp = cscFormat[0]
        ...
        evalstring += ','
        cscFormat_rowInd_tmp = cscFormat[1]
