...
from .mathStructs import Imsl_c_super_lu_smp_factor
...
def superluSmpComplex(a, b, equilibrate=None, columnOrderingMethod=None, colpermVector=None, transpose=None, iterativeRefinement=None, factorSolve=None, diagPivotThresh=None, snodePrediction=None, performanceTuning=None, cscFormat=None, supplySparseLuFactor=None, returnSparseLuFactor=None, condition=None, pivotGrowthFactor=None, forwardErrorBound=None, backwardError=None):
    ...
    """
    imslmath.imsl_z_superlu_smp.restype = POINTER(d_complex)
    shape = []
    evalstring = 'imslmath.imsl_z_superlu_smp('
    evalstring += 'c_int(n)'
    evalstring += ','
    evalstring += 'c_int(nz)'
    evalstring += ','
    ...
    if not (cscFormat is None):
        ...
        cscFormat_hbValues_tmp = toNumpyArray(
            cscFormat_hbValues_tmp, 'hbValues', shape=shape, dtype='d_complex', expectedShape=(0))
        evalstring += 'cscFormat_hbValues_tmp'
