...
from .mathStructs import Imsl_d_super_lu_smp_factor
...
def superluSmp(a, b, equilibrate=None, columnOrderingMethod=None, colpermVector=None, transpose=None, iterativeRefinement=None, factorSolve=None, diagPivotThresh=None, snodePrediction=None, performanceTuning=None, cscFormat=None, supplySparseLuFactor=None, returnSparseLuFactor=None, condition=None, pivotGrowthFactor=None, forwardErrorBound=None, backwardError=None):
    """ Computes the LU factorization of a general sparse matrix by a leftlooking column method using OpenMP parallelism, and solves the real sparse linear system of equations Ax = b.
    """
    imslmath.imsl_d_superlu_smp.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslmath.imsl_d_superlu_smp('
    evalstring += 'c_int(n)'
    evalstring += ','
    evalstring += 'c_int(nz)'
    evalstring += ','
    ...
    if not (colpermVector is None):
        ...
        evalstring += ','
        colpermVector = toNumpyArray(
            colpermVector, 'colpermVector', shape=shape, dtype='int', expectedShape=(n))
        evalstring += 'colpermVector.ctypes.data_as(c_void_p)'
    ...
    if not (snodePrediction is None):
        evalstring += ','
        evalstring += repr(IMSL_SNODE_PREDICTION)
        evalstring += ','
        evalstring += 'c_int(snodePrediction)'
    if not (performanceTuning is None):
        evalstring += ','
        evalstring += repr(IMSL_PERFORMANCE_TUNING)
        evalstring += ','
        performanceTuning = toNumpyArray(
            performanceTuning, 'performanceTuning', shape=shape, dtype='int', expectedShape=(8))
        evalstring += 'performanceTuning.ctypes.data_as(c_void_p)'
    if not (cscFormat is None):
        ...
        cscFormat_hbColPtr_tmp = toNumpyArray(
            cscFormat_hbColPtr_tmp, 'hbColPtr', shape=shape, dtype='int', expectedShape=(0))
        ...
        cscFormat_hbRowInd_tmp = toNumpyArray(
            cscFormat_hbRowInd_tmp, 'hbRowInd', shape=shape, dtype='int', expectedShape=(0))
        ...
        cscFormat_hbValues_tmp = toNumpyArray(
            cscFormat_hbValues_tmp, 'hbValues', shape=shape, dtype='double', expectedShape=(0))
    ...
    if not (supplySparseLuFactor is None):
        ...
        evalstring += ','
        # supplySparseLuFactor = toNumpyArray(supplySparseLuFactor, 'supplySparseLuFactor', shape=shape, dtype='struct', expectedShape=(1))
        # evalstring +='supplySparseLuFactor.ctypes.data_as(c_void_p)'
    ...
    if not (returnSparseLuFactor is None):
        ...
        evalstring += ','
        # returnSparseLuFactor_luFactorReturned_tmp = struct(struct())
        # evalstring += 'byref(returnSparseLuFactor_luFactorReturned_tmp)'
        returnSparseLuFactor_luFactorReturned_tmp = (
            Imsl_d_super_lu_smp_factor)()
        evalstring += 'byref(returnSparseLuFactor_luFactorReturned_tmp)'
