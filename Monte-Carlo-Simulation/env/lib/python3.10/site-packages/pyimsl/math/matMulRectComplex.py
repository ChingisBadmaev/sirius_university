...
###########################################################################
from pyimsl.util.imslUtils import MATH, checkForStr, d_complex, fatalErrorCheck, loadimsl, processRet, toNumpyArray, toByte
...
def matMulRectComplex(string, aMatrix=None, aColDim=None, bMatrix=None, bColDim=None, xVector=None, yVector=None, returnColDim=None):
    ...
    """
    imslmath.imsl_z_mat_mul_rect.restype = POINTER(d_complex)
    shape = []
    evalstring = 'imslmath.imsl_z_mat_mul_rect('
    ...
    if not (aMatrix is None):
        ...
        aMatrix_a_tmp = toNumpyArray(
            aMatrix, 'aMatrix', shape=shape, dtype='d_complex', expectedShape=(0, 0))
    ...
    if not (bMatrix is None):
        ...
        bMatrix_b_tmp = toNumpyArray(
            bMatrix, 'bMatrix', shape=shape, dtype='d_complex', expectedShape=(0, 0))
    ...
    if not (yVector is None):
        ...
        yVector_y_tmp = toNumpyArray(
            yVector, 'yVector', shape=shape, dtype='d_complex', expectedShape=(0))
        evalstring += 'yVector_y_tmp'
