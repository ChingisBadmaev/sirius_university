...
def lattice(nLocations, nReps, nBlocks, nTreatments, rep, block, treatment, y, locations=None, nMissing=None, cv=None, grandMean=None, treatmentMeans=None, stdErrors=None, locationAnovaTable=None, anovaRowLabels=None):
    """ Analyzes balanced and partially-balanced lattice experiments.
    """
    imslstat.imsls_d_lattice.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslstat.imsls_d_lattice('
    evalstring += 'c_int(n)'
    evalstring += ','
    evalstring += 'c_int(nLocations)'
    evalstring += ','
    evalstring += 'c_int(nReps)'
    evalstring += ','
    ...
    if not (stdErrors is None):
        processRet(stdErrors_stdErr_tmp, shape=(4), pyvar=stdErrors)
    if not (locationAnovaTable is None):
