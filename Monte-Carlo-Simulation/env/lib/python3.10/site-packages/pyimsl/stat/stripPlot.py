...
def stripPlot(nLocations, nStripA, nStripB, block, stripA, stripB, y, locations=None, nMissing=None, cv=None, grandMean=None, stripPlotAMeans=None, stripPlotBMeans=None, treatmentMeans=None, stdErrors=None, nBlocks=None, locationAnovaTable=None, anovaRowLabels=None):
    """ Analyzes data from strip-plot experiments.
    """
    imslstat.imsls_d_strip_plot.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslstat.imsls_d_strip_plot('
    evalstring += 'c_int(n)'
    evalstring += ','
    ...
    if not (treatmentMeans is None):
        processRet(treatmentMeans_treatmentMeans_tmp, shape=(
            nStripA, nStripB), pyvar=treatmentMeans)
    if not (stdErrors is None):
    ...
    if not (locationAnovaTable is None):
        processRet(locationAnovaTable_locationAnovaTable_tmp,
                   shape=(nLocations, 12, 6), pyvar=locationAnovaTable)
    if not (anovaRowLabels is None):
        nAnova = 12
        processRet(anovaRowLabels_anovaRowLabels_tmp,
                   shape=(nAnova), pyvar=anovaRowLabels)
    return processRet(result, shape=(12, 6), result=True)
