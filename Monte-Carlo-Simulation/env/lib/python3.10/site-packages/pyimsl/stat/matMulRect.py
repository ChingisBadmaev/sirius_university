...
###########################################################################
from pyimsl.util.imslUtils import STAT, checkForStr, fatalErrorCheck, loadimsl, processRet, toNumpyArray, toByte
from numpy import double, dtype, shape
from ctypes import POINTER, c_double, c_int, c_void_p
from pyimsl.math.matMulResultSize import matMulResultSize
...
def matMulRect(string, aMatrix=None, aColDim=None, bMatrix=None, bColDim=None, xVector=None, yVector=None, returnColDim=None):
    ...
    """
    imslstat.imsls_d_mat_mul_rect.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslstat.imsls_d_mat_mul_rect('
    ...
    if not (bMatrix is None):
        evalstring += ','
        evalstring += repr(IMSLS_B_MATRIX)
        evalstring += ','
    ...
    if not (bColDim is None):
        evalstring += ','
        evalstring += repr(IMSLS_B_COL_DIM)
        evalstring += ','
        evalstring += 'c_int(bColDim)'
    if not (xVector is None):
        evalstring += ','
        evalstring += repr(IMSLS_X_VECTOR)
        evalstring += ','
    ...
    if not (yVector is None):
        evalstring += ','
        evalstring += repr(IMSLS_Y_VECTOR)
        evalstring += ','
    ...
    if not (returnColDim is None):
        evalstring += ','
        evalstring += repr(IMSLS_RETURN_COL_DIM)
        evalstring += ','
