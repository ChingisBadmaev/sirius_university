...
def randomExponentialMix(nRandom, theta1, theta2, p):
    """ Generates pseudorandom numbers from a mixture of two exponential distributions.
    """
    imslstat.imsls_d_random_exponential_mix.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslstat.imsls_d_random_exponential_mix('
    evalstring += 'c_int(nRandom)'
    evalstring += ','
    evalstring += 'c_double(theta1)'
    evalstring += ','
    evalstring += 'c_double(theta2)'
    evalstring += ','
