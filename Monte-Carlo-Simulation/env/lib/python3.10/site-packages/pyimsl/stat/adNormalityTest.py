...
def adNormalityTest(x, stat=None, nMissing=None):
    imslstat.imsls_d_ad_normality_test.restype = c_double
    shape = []
    evalstring = 'imslstat.imsls_d_ad_normality_test('
    ...
    if not (stat is None):
        ...
        evalstring += ','
        stat_adstat_tmp = c_double()
        evalstring += 'byref(stat_adstat_tmp)'
    ...
    if not (stat is None):
        processRet(stat_adstat_tmp, shape=(1), pyvar=stat)
    if not (nMissing is None):
