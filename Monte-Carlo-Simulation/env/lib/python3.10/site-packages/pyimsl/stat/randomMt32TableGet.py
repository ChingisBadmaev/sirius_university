...
def randomMt32TableGet(table):
    VersionFacade.checkVersion(6)
    imslstat.imsls_random_MT32_table_get.restype = None
    checkForList(table, 'table')
    evalstring = 'imslstat.imsls_random_MT32_table_get('
    table_tmp = POINTER(c_int)(c_int())
    evalstring += 'byref(table_tmp)'
    evalstring += ',0)'
    result = eval(evalstring)
    fatalErrorCheck(STAT)
    processRet(table_tmp, shape=(625), pyvar=table)
    return
