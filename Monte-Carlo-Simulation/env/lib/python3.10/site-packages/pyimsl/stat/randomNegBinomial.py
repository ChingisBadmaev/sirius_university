...
def randomNegBinomial(nRandom, rk, p):
    """ Generates pseudorandom numbers from a negative binomial distribution.
    """
    imslstat.imsls_d_random_neg_binomial.restype = POINTER(c_int)
    shape = []
    evalstring = 'imslstat.imsls_d_random_neg_binomial('
    evalstring += 'c_int(nRandom)'
    evalstring += ','
    evalstring += 'c_double(rk)'
    evalstring += ','
