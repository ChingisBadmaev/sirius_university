...
def mlffNetworkRead(filename, t_print=None, file=None):
    VersionFacade.checkVersion(7)
    imslstat.imsls_d_mlff_network_read.restype = POINTER(Imsls_d_NN_Network)
    shape = []
    evalstring = 'imslstat.imsls_d_mlff_network_read('
    if not (file is None):
        evalstring += 'None'
    else:
