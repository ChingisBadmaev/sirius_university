...
def randomMt32Init(key):
    VersionFacade.checkVersion(6)
    imslstat.imsls_random_MT32_init.restype = None
    shape = []
    evalstring = 'imslstat.imsls_random_MT32_init('
    evalstring += 'c_int(keyLength)'
    evalstring += ','
    key = toNumpyArray(key, 'key', shape=shape, dtype='int', expectedShape=(0))
    evalstring += 'key.ctypes.data_as(c_void_p)'
    keyLength = shape[0]
    evalstring += ')'
