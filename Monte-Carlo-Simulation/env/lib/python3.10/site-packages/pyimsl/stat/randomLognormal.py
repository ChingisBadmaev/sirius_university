...
from numpy import mean, shape, std
from ctypes import POINTER, c_double, c_int

imslstat = loadimsl(STAT)


def randomLognormal(nRandom, mean, std):
    """ Generates pseudorandom numbers from a lognormal distribution.
    """
    imslstat.imsls_d_random_lognormal.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslstat.imsls_d_random_lognormal('
    evalstring += 'c_int(nRandom)'
    evalstring += ','
    evalstring += 'c_double(mean)'
    evalstring += ','
    evalstring += 'c_double(std)'
    evalstring += ',0)'
