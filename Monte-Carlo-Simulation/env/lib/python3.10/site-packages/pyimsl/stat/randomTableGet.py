...
def randomTableGet(table):
    """ Retrieves the current table used in the shuffled generator.
    """
    imslstat.imsls_d_random_table_get.restype = None
    shape = []
    evalstring = 'imslstat.imsls_d_random_table_get('
    checkForList(table, 'table')
    table_tmp = POINTER(c_double)(c_double())
    evalstring += 'byref(table_tmp)'
    evalstring += ',0)'
    result = eval(evalstring)
    fatalErrorCheck(STAT)
    processRet(table_tmp, shape=(128), pyvar=table)
    return
