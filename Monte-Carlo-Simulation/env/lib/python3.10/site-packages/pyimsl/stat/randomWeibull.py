...
def randomWeibull(nRandom, a, b=None):
    """ Generates pseudorandom numbers from a Weibull distribution.
    """
    imslstat.imsls_d_random_weibull.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslstat.imsls_d_random_weibull('
    evalstring += 'c_int(nRandom)'
    evalstring += ','
    evalstring += 'c_double(a)'
    if not (b is None):
        evalstring += ','
        evalstring += repr(IMSLS_B)
        evalstring += ','
