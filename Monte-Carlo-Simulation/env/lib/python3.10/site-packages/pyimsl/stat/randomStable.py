...
def randomStable(nRandom, alpha, bprime):
    """ Generates pseudorandom numbers from a stable distribution.
    """
    imslstat.imsls_d_random_stable.restype = POINTER(c_double)
    shape = []
    evalstring = 'imslstat.imsls_d_random_stable('
    evalstring += 'c_int(nRandom)'
    evalstring += ','
    evalstring += 'c_double(alpha)'
    evalstring += ','
    evalstring += 'c_double(bprime)'
    evalstring += ',0)'
