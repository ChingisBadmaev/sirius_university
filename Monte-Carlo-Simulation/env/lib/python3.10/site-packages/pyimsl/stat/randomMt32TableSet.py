...
def randomMt32TableSet(table):
    VersionFacade.checkVersion(6)
    imslstat.imsls_random_MT32_table_set.restype = None
    shape = []
    evalstring = 'imslstat.imsls_random_MT32_table_set('
    table = toNumpyArray(table, 'table', shape=shape,
                         dtype='int', expectedShape=(625))
    if (table[0] is None):
        evalstring += 'None'
    else:
