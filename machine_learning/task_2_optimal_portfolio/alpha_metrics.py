...
def get_returns(returns, data):
    return data / data.shift(1) - 1


def get_turnover(alpha, isyear=1):
...
def get_pnl(returns, alpha, isyear=1):
...
def alpha_stats(data_returns, alpha, is_plot=False):
    days_pnl = get_pnl(data_returns, alpha, 0)['pnl']
    print('pnl allTime:', days_pnl.sum())
